...
import { getOneUserById } from "../../redux/users/usersAction";
...
function OneUserPage() {
  const { id } = useParams();
  const { loading, users, oneUser, error } = useSelector(
    (state) => state.usersList
  );
  const dispatch = useDispatch();

  console.log({ loading, users, oneUser, error });
  useEffect(() => {
    console.log(id);
  ...
  return (
    <div className="w-2/3 p-3 mt-10 max-md:w-full">

      <div class="p-4 flex flex-col items-center ">
        <div class="mb-4 text-center opacity-90">
          <div className="h-40 w-40 bg-gray-600 rounded-full flex justify-center items-center mr-2">
            <svg
      ...
      <div className="w-full p-3 mt-10 grid grid-cols-3 gap-3 max-md:grid-cols-1 max-md:w-full">
      {oneUser &&
        oneUser.posts.map((post) => (
          <PostCard key={post._id} user={oneUser.user} title={post.title} body={post.body} />
        ))}
    </div>
    </div>
  );
